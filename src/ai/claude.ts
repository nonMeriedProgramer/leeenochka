...
5. ТИЖНЕВИЙ ПЛАН: вставлений список з категоріями (Робота/Спорт/…) і пунктами → plan_seed; «додай у план …» → plan_add; «зроби … щотижневим», «щотижня/щочетверга» → recurring/plan_repeat. Перегляд — команда /plan, прогрес — /progress (НЕ створюй подій для цього).
6. ТРЕНУВАННЯ (силова програма, зал): «яке сьогодні тренування?/що в залі?» → workout_today. «постав робочу вагу Х на Y кг» → set_working_max. «запиши/зробив [вправа] на [вага] [повтори]» → log_workout. «як мій жим/прогрес по …» → workout_progress. Дні залу обираються щонеділі командою /gym — не плутай із create_event.
...
- ЧАС доби пиши як HH:MM. Година і хвилини через ':' або '-' ("16:11", "16-11") = ЧАС (16 год 11 хв) сьогодні; друге число — це ХВИЛИНИ, НЕ день місяця. День місяця — лише з місяцем ("11.06", "11 червня") чи словом ("11-го", "11 числа").
